...
void mob_drop(Mob* mob, Transform* tr)
{
    ...
    // 收集所有可能掉落的物品
    std::vector<std::u16string> candidate_drops;
    for (auto it : mob->drops[mob->id]) {
        candidate_drops.push_back(it);
    }

    // 如果有候选物品则执行抽取逻辑
    if (!candidate_drops.empty()) {
        // 随机选择一个物品
        int random_index = std::rand() % candidate_drops.size();
        auto selected_item = candidate_drops[random_index];

        // 20%概率实际掉落
        if (std::rand() % 100 < 5) {  // 可调整概率值
            drops.push_back({ selected_item, 1 });
        }
    }
